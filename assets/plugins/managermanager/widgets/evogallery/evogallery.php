...
function mm_widget_evogallery($moduleid, $title='', $roles='', $templates='') {
        ...
        if ($e->name == 'OnDocFormRender' && useThisRule($roles, $templates)) {
                ...
                if (isset($content['id']))
                        $iframecontent = '<iframe id="mm_evogallery" src="'.$modx->config['site_url'].MGR_DIR.'/index.php?a=112&id='.$moduleid.'&onlygallery=1&action=view&content_id='.$content['id'].'" style="width:100%;height:600px;" scrolling="auto" frameborder="0"></iframe>';
                else
